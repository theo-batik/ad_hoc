...
H_LOW = int(getenv("H_LOW"))
H_HIGH = int(getenv("H_HIGH"))
print(H_HIGH)

# Grayscale intensity filter to binary image
INTENSITY_HIGH = 255
INTENSITY_LOW = 66
...
def apply_intensity_filter(gray_image, low, high):
    # Apply thresholding to create a binary mask
    _, binary_image = cv2.threshold(gray_image, low, high, cv2.THRESH_BINARY) # cv.ADAPTIVE_THRESH_GAUSSIAN_C
...
def main():
    ...
    binary_image = apply_intensity_filter(gray_image, low=INTENSITY_LOW, high=INTENSITY_HIGH)
    ...
    write_image(gray_image, 'grayscale')
    write_image(binary_image, 'binary')
